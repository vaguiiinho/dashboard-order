...
export function CadastroSetorForm() {
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<SetorForm>({
    resolver: zodResolver(setorSchema),
    defaultValues: { ativo: true },
  });

  const onSubmit = async (data: SetorForm) => {
    try {
      setLoading(true);
      setError(null);
      await ordemServicoService.createSetor(data);
