...
export function CadastroTipoAtividadeForm() {
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [setores, setSetores] = useState<Setor[]>([]);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<TipoForm>({
  ...
  const onSubmit = async (data: TipoForm) => {
    try {
      setLoading(true);
      setError(null);
      await ordemServicoService.createTipoAtividade(data);
      setSuccess('Tipo de atividade cadastrado com sucesso');
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold dashboard-text-primary mb-2">Cadastro de Tipo de Atividade</h1>
        <p className="dashboard-text-secondary">Crie tipos de atividade vinculados a um setor</p>
      </div>
